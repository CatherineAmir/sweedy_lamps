    ...
    def get_lines(self, productions):
        ...
        for product in productions.mapped('product_id'):
            ...
            if Workorders:
                ...
                query_str = """SELECT w.operation_id, op.name, partner.name, sum(t.duration), op.time_cycle_manual , op.labour_cost_per_hour, op.overhead_cost_per_hour
                                FROM mrp_workcenter_productivity t
                                LEFT JOIN mrp_workorder w ON (w.id = t.workorder_id)
                                LEFT JOIN mrp_workcenter wc ON (wc.id = t.workcenter_id )
                                LEFT JOIN res_users u ON (t.user_id = u.id)
                                ...
                                LEFT JOIN mrp_routing_workcenter op ON (w.operation_id = op.id)
                                WHERE t.workorder_id IS NOT NULL AND t.workorder_id IN %s
                                GROUP BY w.operation_id, op.name, partner.name, t.user_id, op.time_cycle_manual, op.labour_cost_per_hour, op.overhead_cost_per_hour
                                ORDER BY op.name, partner.name
                            """
                self.env.cr.execute(query_str, (tuple(Workorders.ids), ))
                for op_id, op_name, user, duration, time_cycle_manual, labour_cost_per_hour, overhead_cost_per_hour in self.env.cr.fetchall():
                    labour_operations.append([user, op_id, op_name, duration / 60.0,time_cycle_manual or 0, labour_cost_per_hour or 0])
                    overhead_operations.append([user, op_id, op_name, duration / 60.0,time_cycle_manual or 0, overhead_cost_per_hour or 0])
