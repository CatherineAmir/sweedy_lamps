        ...
        for product in productions.mapped('product_id'):
            ...
            if Workorders:
                ...
                query_str = """SELECT w.operation_id, op.name, partner.name, sum(t.duration), op.time_cycle_manual , op.labour_cost_per_hour, op.overhead_cost_per_hour, op.number_labours
                            ...
                            """
                self.env.cr.execute(query_str, (tuple(Workorders.ids), ))
                for op_id, op_name, user, duration, time_cycle_manual, labour_cost_per_hour, overhead_cost_per_hour,number_labours in self.env.cr.fetchall():
                    labour_operations.append([user, op_id, op_name, duration / 60.0,time_cycle_manual/60.0 or 0, labour_cost_per_hour or 0 ,number_labours])
                    overhead_operations.append([user, op_id, op_name, duration / 60.0,time_cycle_manual/60.0 or 0, overhead_cost_per_hour or 0])
