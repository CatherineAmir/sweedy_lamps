        ...
        try:
            with api.Environment.manage():
                ...
                if job['id'] == cron.env.ref('auto_odoo_db_and_file_backup.auto_db_backup_scheduler_sftp').id:
                    ...
                    fields.Datetime.to_string(nextcall.astimezone(pytz.UTC)),
                    bid ))
                if job['id'] == cron.env.ref('auto_odoo_db_and_file_backup.auto_db_backup_scheduler_AWSs3').id:
                    bid = cron.env.ref('auto_odoo_db_and_file_backup.rule_upload_backup_to_AWSs3').id
                    cron_cr.execute("UPDATE database_backup SET next_exec_dt=%s"+" WHERE id=%s",(
                    fields.Datetime.to_string(nextcall.astimezone(pytz.UTC)),
                    bid ))    
                cron.flush()
            ...
            if rec.is_active:
                if rec.backup_destination == 'folder':
                    ...
                    try:
                        ...
                        self._take_dump(self.env.cr.dbname, fp, 'database.backup', rec.backup_type)
                        fp.close()
                        if rec.backup == 'db_and_files':
                            ...
                            with odoo.tools.osutil.tempdir() as dump_dirf:
            ...
            for item in content['files']:
                if self.env.cr.dbname in item['name']:
                    ...
                    if delta1.days >= rec.backup_id.days_to_keep:
                        ...
                        response1 = self.env['google.service']._do_request(url1, params, headers, type='DELETE')
                else:
                    continue
