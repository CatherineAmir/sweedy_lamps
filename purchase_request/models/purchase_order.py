...
class PurchaseOrder(models.Model):
    ...
    purchase_request_id = fields.Many2one(comodel_name="purchase.request", readonly=True )
    requested_by = fields.Many2one(related='purchase_request_id.requested_by',string="Purchase Requested By" )
    request_department_id = fields.Many2one(comodel_name="hr.department", readonly=True )
    ...
    def cancel_request_po(self):
        if self.purchase_request_id:
            purchase_orders = self.env['purchase.order'].search([('purchase_request_id','=',self.purchase_request_id.id)]) - self
            purchase_orders.button_cancel()

    def button_confirm(self):
        self._purchase_request_line_check()
        res = super(PurchaseOrder, self).button_confirm()
        self.cancel_request_po()
