...
class PurchaseRequestLineMakePurchaseOrder(models.TransientModel):
    ...
    def _prepare_purchase_order(self, picking_type, group_id, company, origin):
        ...
        if self._context.get('active_model') == 'purchase.request' and self._context.get('active_id'):
            request = self.env['purchase.request'].browse(self._context.get('active_id'))
            employee = self.env['hr.employee'].search([('user_id', '=', request.requested_by.id)],limit=1)
            data.update({
                'purchase_request_id': request.id,
                'request_department_id': employee.department_id.id,
            })
        return data
