odoo.define('boraq_pos_kitchen_screen.pos',function (require) {
    "use strict";
...
//        	self.pos.mirror_kitchen_orders();
        ...
        order_print_receipt: function(ev){
            ...
            if(data_for === 'line'){
                order_id = [$(ev.currentTarget).data('id'), 'load_order_line_details'];
            }else{
                order_id = [$(ev.currentTarget).data('pos-order-id'), 'load_order_details'];
            }
            if(order_id){
                rpc.query({
                    model: 'pos.order',
